...
The repo now includes `_redirects` so token routes like `/hype`, `/lit`, `/pump`, `/sky`, `/aave`, `/rhea`, `/railgun`, `/ethfi`, `/metadao`, `/mega`, `/ethereum`, `/arbitrum`, `/solana`, `/tron`, `/bsc`, `/near`, `/limitless`, `/opinion`, `/bankr`, `/ore`, `/aerodrome`, `/polygon`, `/maple`, `/meteora`, `/pendle`, `/jito`, `/jupiter`, `/raydium`, `/fluid`, and `/kamino` all rewrite to the shared `index.html` shell.
